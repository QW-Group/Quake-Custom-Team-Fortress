...
// OfN
int numwarnings;
int numunusedwarnings;
int numquakecwarnings;

int pr_warnings;
precache_list models_list[MAX_MODELS];
precache_list sounds_list[MAX_SOUNDS];
...
#define MAX_TRANSFORMVECTOR_STRLEN 512

//==============================================================
// Converts a value_x _y or _z variable name to "value"

char* TransformFloatVectorName(char* name)
{
	static char tstring[MAX_TRANSFORMVECTOR_STRLEN];
	...
	if (len > MAX_TRANSFORMVECTOR_STRLEN -2)
		strcpy(tstring,"Error (DefName too long)");
	else
	{
		for (i = 0; i < (len-2) && name[i]; i++)
		{
			tstring[i]=name[i];
		}
...
//==========================================================
// Same as strstr() but returns a boolean

boolean Q_strstr(char* str1, char* str2)
{
	if (!str2 || str2[0]=='\0' || !str1 || str1[0]=='\0')
		return false;
	...
	while (str1[counter])
	{
		...
		{			
			...
			{			
				...
				if (!str1[counter + counter2])
					return false;

				if (str2[counter2] != str1[counter + counter2])
					break;
...
// look for progs
	if (def = PR_GetDef(&type_entity, "progs", NULL, false, NULL))
	{
...
int main (int argc, char **argv)
{
	...
	printf("CPQCCX v1.1 by OfteN[cp] for Prozac CustomTF (based on qccx)\n");
...
// compile all the files
	do
	{
		...
		if (!fastmode)
			printf("%s (%d lines) compiled.\n", filename,pr_source_line);
	...
	if (pr_warnings)
	{
		...
		printf("Checking unused globals...\n");

		for (p = 0; p < numpr_globals; p++)
		{
			def = pr_global_defs[p];
			...
			if (!strcmp(def->name,"IMMEDIATE") || !strcmp(def->name,"temp"))
				continue;
			...
			// check if a global used by C code
			itemindex = 0;
			
			while (globals_c[itemindex])
			{
				if (!strcmp(globals_c[itemindex],def->name))
				{
					itemindex = 999999;
					break;
				}
			...
			if (def->owned_by)
			{
				PR_RaiseWarning(WARNINGTYPE_UNUSED,def->source_file,def->source_line,"Global vector \"%s\" is never used",TransformFloatVectorName(def->name));
			}
			else
			{
				if (def->type->type == ev_field)
					PR_RaiseWarning(WARNINGTYPE_UNUSED,def->source_file,def->source_line,"Entity field \"%s\" is never used",def->name);
				else
					PR_RaiseWarning(WARNINGTYPE_UNUSED,def->source_file,def->source_line,"Global \"%s\" is never used",def->name);
			}
		}

		printf("Checking unused functions...\n");
		...
		for (p = 0; p < numpr_globals; p++)
		{
			...
			// Check to skip spawn functions
			if (!strncmp(def->name,"func_",5)
				||!strncmp(def->name,"info_",5)
				||!strncmp(def->name,"ambient_",8)
				||!strncmp(def->name,"item_",5)
				||!strncmp(def->name,"effect_",7)
				||!strncmp(def->name,"misc_",5)
				||!strncmp(def->name,"light_",6)
				||!strncmp(def->name,"trap_",5)
				||!strncmp(def->name,"path_",5)
				||!strncmp(def->name,"weapon_",7)
				||!strncmp(def->name,"trigger_",8)
				||!strncmp(def->name,"model_",6)
				||!strncmp(def->name,"rotate_",7)
				||!strcmp(def->name,"air_bubbles")
				||!strcmp(def->name,"light"))
				continue;

			// check if a function called by C code
			itemindex = 0;
			
			while (functions_c[itemindex])
			{
				if (!strcmp(functions_c[itemindex],def->name))
				{
					itemindex = 999999;
					break;
				}
			...
			PR_RaiseWarning(WARNINGTYPE_UNUSED,def->source_file,def->source_line,"Function \"%s\" is never used",def->name);
		}
		
		printf("Checking unused models/sounds...\n");

		for (p = 0; p < nummodels; p++)
			if (!models_list[p].is_used)
				PR_RaiseWarning(WARNINGTYPE_UNUSED,models_list[p].source_file,models_list[p].source_line,"Precacheed model \"%s\" is never used",precache_models[p]);
		
		for (p = 0; p < numsounds; p++)
			if (!sounds_list[p].is_used)
				PR_RaiseWarning(WARNINGTYPE_UNUSED,sounds_list[p].source_file,sounds_list[p].source_line,"Precacheed sound \"%s\" is never used",precache_sounds[p]);
...
// write progdefs.h
	...
	if (pr_warnings) // Display warnings if enabled
		printf("%d warnings < QuakeC: %d issues Unused: %d items\n",numwarnings,numquakecwarnings,numunusedwarnings);
