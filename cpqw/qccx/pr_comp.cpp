...
// OfN
def_t* lastfunc;
boolean lastprecacheready;

char* precache_sounds_c[] =
{
///*
"wizard/hit.wav",
"hknight/hit.wav",
...
{	
	...
	for (cell = htable[pr_immediate_index]; cell != NULL; cell = cell->next) 
	{
		...
		// OfN
		if (pr_immediate_type == &type_string)
			CheckPrecacheStrings(pr_immediate_string);
		// OfN end
		return cn;
	}
...
//==========================================================
// OfN - Checks if the string uses a model/sound
// AND (like 2 diff funcs) checks if a model is already used

void CheckPrecacheStrings(char* str)
{
	if (!pr_warnings)
		return;
	
	if (!str || str[0]=='\0' || strlen(str) < 2)
		return;

	// When a precache function is used scan all the strings for matches
	if (lastfunc)
	{
		int z; char* tmp;
		tmp = strings;		

		if (!strncmp(lastfunc->name,"precache_sound", 14))
		{
			for (z = 0; z < strofs; z++)
			{
				if (strings[z] == '\0')
		...
		else if (!strncmp(lastfunc->name,"precache_model", 14))
		{
			for (z = 0; z < strofs; z++)
			{
				if (strings[z] == '\0')
	...
	// Check when a new string is added if matches any precache
	if (pr_warnings)
	{
		int z; boolean done = false;
				
		for (z=0; z < nummodels && !done; z++)
			if (!models_list[z].is_used)
				if (Q_strstr(str,precache_models[z]))
				{
					models_list[z].is_used = true;
					done = true;
				}
			
		for (z=0; z < numsounds && !done; z++)
			if (!sounds_list[z].is_used && str != precache_sounds[z])
				if (Q_strstr(str,precache_sounds[z]))
				{					
					sounds_list[z].is_used = true;
					done = true;				
				}		
	}
}
...
void PrecacheSound (def_t *e, int ch)
{
	...
	if (pr_warnings)
	{
		sounds_list[i].is_used = false;
		sounds_list[i].source_line = pr_source_line;
		
		sounds_list[i].source_file = strings + s_file;

		if (!strlen(n))
		{
			// FIX?
			numsounds--;
			precache_sounds[i][0]='\0';
			sounds_list[i].is_used = true;
			lastprecacheready = false;
			return;
		}		

		// If this precache is already in the strings set our used flag to true
		if (lastprecacheready)
		{
			sounds_list[i].is_used = true;
			lastprecacheready = false;
			return;
		...
		// check if a sound needed by C code
		int indexprec = 0;
		
		while (precache_sounds_c[indexprec])
		{
			if (!strcmp(precache_sounds_c[indexprec],n))
			{
				sounds_list[i].is_used = true;
				return;
			}
...
void PrecacheModel (def_t *e, int ch)
{
	...
	if (pr_warnings)
	{
		models_list[i].is_used = false;
		models_list[i].source_line = pr_source_line;
		
		models_list[i].source_file = strings + s_file;

		if (!strlen(n))
		{
			models_list[i].is_used = true;
			nummodels--;
			precache_models[i][0]='\0';
			lastprecacheready = false;
			return;
		}
		...
		// check if a model needed by C code
		int indexprec = 0;
		
		while (precache_models_c[indexprec])
		{
			if (!strcmp(precache_models_c[indexprec],n))
			{
				models_list[i].is_used = true;
				return;
			}
...
// copy the arguments to the global parameter variables
	...
	if (!PR_Check(")"))
	{
		do
		{
			...
			if (!PR_Check("#"))
			{
				// OfN set our pointer to current function
				if (func->name)
				{
					if (!strncmp(func->name,"precache_sound", 14)
						|| !strncmp(func->name,"precache_model", 14))
					{
						lastfunc = func;
						lastprecacheready = false;
					}
...
// look through the defs
	d = PR_GetDef (NULL, name, pr_scope, false, NULL);
	if (!d)
...
void PR_ParseState (void)
{
	...
	name = PR_ParseName ();
	def = PR_GetDef (&type_function, name,0, true, NULL);

	// OfN - Special handling of is_used for functions used on states, (they dont need prior declaration)
	def->is_used = true;
...
//
	...
	{
		defs[i] = PR_GetDef (type->parm_types[i], pr_parm_names[i], pr_scope, true, NULL);
...
// emit an end of statements opcode
	...
	// OfN - Check unused locals
	if (pr_warnings)
	{
		def_t	*def; int z;
			
		if (locals_end - locals_start != 0)
		{			
			for (i = locals_start; i < locals_end && i < numpr_globals; i++)
			{
				def = pr_global_defs[i];

				if (!def)
					continue;

				if (def->scope != pr_scope)
					continue;

				if (def->type->type != ev_string
					&& def->type->type != ev_entity
					&& def->type->type != ev_vector
					&& def->type->type != ev_float)
					continue;

				for (z = 0; z < type->num_parms; z++)
				{
					if (defs[z])
					if (defs[z] == def)
						z = 999;
				...
				if (def->owned_by)
				{
					if (def->owned_by == def && !def->is_used)
						PR_RaiseWarning(WARNINGTYPE_UNUSED|WARNINGTYPE_QUAKEC,NULL,def->source_line,"Local vector \"%s\" is never used",TransformFloatVectorName(def->name));
				}
				else if (!def->is_used)
					PR_RaiseWarning(WARNINGTYPE_UNUSED|WARNINGTYPE_QUAKEC,NULL,def->source_line,"Local variable \"%s\" is never used",def->name);		
			}
		}
	}	
	// OfN end
...
============
*/
def_t *PR_GetDef (type_t *type, char *name, def_t *scope, boolean allocate, getdef_extras_t* extra_info)
{	
	def_t	*def;
	...
	for (cell = htable[index]; cell != NULL; cell = cell->next) 
	{
		...
		if ( !STRCMP(def->name, name) ) 
		{
			...
			if (extra_info)
			{
				extra_info->was_used = def->is_used;
				extra_info->allocation_done = false;
			}

			if (def->owned_by)
			{
				static def_t* tmpdef;
				tmpdef = (def_t*)def->owned_by;
				tmpdef->is_used = true;
			}
...
// allocate a new def
	...
	if (extra_info)
	{
		extra_info->was_used = false;
		extra_info->allocation_done = true;
	}

	def->is_used = false;
	def->source_file = strings + s_file;
	def->source_line = pr_source_line;
	def->owned_by = NULL;
	// OfN end
...
//
	...
	{		
		...
		tmpdef = PR_GetDef (&type_float, element, scope, true, NULL);
		tmpdef->owned_by = tmpdef;

		def->owned_by = tmpdef;
		
		sprintf (element, "%s_y",name);
		PR_GetDef (&type_float, element, scope, true, NULL)->owned_by = tmpdef;
	...
	if (type->type == ev_field)
	{
		*(int *)&pr_globals[def->ofs] = pr.size_fields;
		...
		if (type->aux_type->type == ev_vector)
		{
			...
			sprintf (element, "%s_x",name);
			tmpdef = PR_GetDef (&type_floatfield, element, scope, true, NULL);
			tmpdef->owned_by = tmpdef;

			def->owned_by = tmpdef;
			
			sprintf (element, "%s_y",name);
			PR_GetDef (&type_floatfield, element, scope, true, NULL)->owned_by = tmpdef;
...
void PR_ParseDefs (void)
{
	...
	dfunction_t	*df;
	int			i;
	//int			locals_start; // OfN moved to global scope

	// OfN
	getdef_extras_t extra_info;
	...
	do
	{
		...
		def = PR_GetDef (type, name, pr_scope, true, &extra_info);

		// OfN - Reset is_used flag for function declarations and initializations
		if (type->type == ev_function)
		{
			if (extra_info.was_used == false)
				def->is_used = false;
		}
