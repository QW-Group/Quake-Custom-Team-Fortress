...
Reports a warning about the code
===========
*/

void PR_RaiseWarning (int warningflags, char* sourcefile, int sourceline, char *warning, ...)
{
	va_list		argptr;
	char		string[1024];
	...
	if (sourceline == 0)
		sourceline = pr_source_line;

	if (sourcefile == NULL)
		sourcefile = strings + s_file;

	printf ("%s:%i:WARNING! %s\n", sourcefile, sourceline, string);

	numwarnings++;

	if (warningflags & WARNINGTYPE_UNUSED)
		numunusedwarnings++;
	if (warningflags & WARNINGTYPE_QUAKEC)
		numquakecwarnings++;
}
