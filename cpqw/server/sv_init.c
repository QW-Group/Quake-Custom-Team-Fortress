...
// OfN
Pinmsg_Item pinmsg[NUM_PINMSGS];
char pinmsg_cmdbuffer[MAX_PINMSGITEM_STRING];

/*WhiteWolf: custom ent support */
extern  cvar_t  sv_loadentfiles = {"sv_loadentfiles", "0"};
...
{
	edict_t		*ent;
	char *entitystring; /* WhiteWolf: custom .ent support */
	int			i;
	...
	// OfN - Reset string buffers, and "permanent" strings
	ResetStrBuffers();
	ResetPermStrings();
...
//ZOID - make sure we update frags right
	...
	/*WhiteWolf: custom ent support ED_LoadFromFile (sv.worldmodel->entities); */
	entitystring = NULL;
	if(sv_loadentfiles.value) {
		entitystring = (char *)COM_LoadHunkFile(va("maps/%s.ent", sv.name));
		if(entitystring) {
			Con_DPrintf("Using entfile maps/%s.ent\n", sv.name);
			Info_SetValueForStarKey(svs.info, "*entfile", va("%i", CRC_Block((byte *)entitystring, com_filesize)), MAX_SERVERINFO_STRING);
		}
	}
	if(!entitystring) {
