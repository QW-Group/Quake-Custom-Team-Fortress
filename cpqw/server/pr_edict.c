...
dstatement7_t	*pr_statements7;			// KK prog version 7+
...
// flush the non-C variable lookup cache
	...
	progsname = Info_ValueForKey(localinfo, "progs"); // KK teste servers
	if (!*progsname || !strcmp(progsname,"0"))
		//progsname = "qwprogs.dat";
		progsname = "prozac.dat"; // OfN
...
// byte swap the header
	...
	if (progs->version == PROG_VERSION) {
		Sys_Printf ("Loaded Pre-Version7 qwprogs.dat\n");
		pr_statements = (dstatement_t *)((byte *)progs + progs->ofs_statements);
	} else if (progs->version == PROG_VERSION2) {
		//Sys_Printf ("Loaded Version7+ progs.dat, support by KK.\n");
		Sys_Printf ("Loaded Version7+ qwprogs.dat, original concept by KK.\n");
		pr_statements7 = (dstatement7_t *)((byte *)progs + progs->ofs_statements);
	} else
		SV_Error ("qwprogs.dat has wrong version number (%i should be %i or %i)", progs->version, PROG_VERSION, PROG_VERSION2);

	if (progs->crc != PROGHEADER_CRC)
	//	SV_Error ("You must have the progs.dat from QuakeWorld installed");
		SV_Error ("Missmatch on qwprogs.dat header CRC, this server can only run QuakeWorld mods!"); // OfN
	...
	Con_DPrintf ("num f=0x%X s=0x%X gd=0x%X fd=0x%X st=0x%X g=0x%X\n", //KK - OfN was Sys_Printf
			(unsigned int)progs->numfunctions,
			(unsigned int)progs->numstrings,
			(unsigned int)progs->numglobaldefs,
			(unsigned int)progs->numfielddefs,
			(unsigned int)progs->numstatements,
			(unsigned int)progs->numglobals);
// byte swap the lumps
	for (i=0 ; pr_statements && i<progs->numstatements ; i++)
	{
	...
	for (i=0 ; pr_statements7 && i<progs->numstatements ; i++)
	{
		pr_statements7[i].op = LittleShort(pr_statements7[i].op);
		pr_statements7[i].a = LittleLong(pr_statements7[i].a);
		pr_statements7[i].b = LittleLong(pr_statements7[i].b);
		pr_statements7[i].c = LittleLong(pr_statements7[i].c);
	}
	...
	if ((f = ED_FindFunction ("ClientCommand")) != NULL) //OfN
