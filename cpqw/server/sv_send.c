...
void Con_Printf (char *fmt, ...)
{
	...
	// OfN - Moved to Sys_PRintf
	/*if (sv_logfile)
		fprintf (sv_logfile, "%s", msg);*/

	// OfN - Write also to crash log if aproppiate
	if (crashlog_running)
		CrashLog_Append(msg);
}
...
qboolean dont_sysprint_in_bdcast = false;
...
void SV_BroadcastPrintf (int level, char *fmt, ...)
{
	...
	if (dont_sysprint_in_bdcast)		// for KK color support
		dont_sysprint_in_bdcast = false;
	else
...
void SV_UpdateClientStats (client_t *client)
{
	...
	//stats[STAT_ITEMS] = (int)ent->v.items | ((int)client->runes<<28);
	
	//stats[STAT_ITEMS] = (int)ent->v.items | ((int)client->edict->v.runes_owned<<28);
	
	if (runes_owned_offset)
	{
		eval_t* val;
		val = (eval_t *)((char *)&client->edict->v + runes_owned_offset);
		stats[STAT_ITEMS] = (int)ent->v.items |	(int)val->_float<<28;		
	}
	else // Mod that doesnt use a "runes_owned" field.. (standard behaviour)
		stats[STAT_ITEMS] = (int)ent->v.items | ((int)pr_global_struct->serverflags << 28);
...
// check for changes to be sent over the reliable streams to all clients
	for (i=0, host_client = svs.clients ; i<MAX_CLIENTS ; i++, host_client++)
	{
		...
		if (host_client->old_frags != (int)host_client->edict->v.frags) // (int)Fixed overflows? OfN
		{
			for (j=0, client = svs.clients ; j<MAX_CLIENTS ; j++, client++)
			{
