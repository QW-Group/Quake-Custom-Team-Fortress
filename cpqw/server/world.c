...
void SV_ClipToLinks ( areanode_t *node, moveclip_t *clip )
{
	...
	{
		touch = NEXT_EDICT(sv.edicts);
		for (i=1 ; i<sv.num_edicts ; i++, touch = NEXT_EDICT(touch))
		{
			if (touch->free)
				continue;                 
		...
		return; // No need to continue at all (we just scanned all edicts!)
	}
	//OfN - Trigger traceline support
