...
cvar_t sv_maxrate = {"sv_maxrate", "0"};	// KK server maxrate
cvar_t sv_cheatpc = {"sv_cheatpc", "140"};	// KK percentage allowed
cvar_t admin_password = {"admin_password", ""};	// KK admin password
void KK_stuffcmd (client_t *cl, char *str);	// KK 
void SV_SavePenaltyFilter(client_t *cl, filtertype_t ft, double pentime);
double SV_RestorePenaltyFilter(client_t *cl, filtertype_t ft);

// OfN
cvar_t sv_pinsfile = {"sv_pinsfile","pinmsgs.pm"};
cvar_t sv_crashlog = {"sv_crashlog","0"};
cvar_t sv_restarttime = {"sv_restarttime","0"};
cvar_t sv_updating = {"sv_updating","0"};
...
void SV_DropClient (client_t *drop)
{
	...
	// KK save penalties through disconnects
	...
	Sys_BeginColor();
	Sys_ConsoleColor(COLOR_FORE_DROP,COLOR_BACK_DROP);

	if (drop->spectator)
	    //Sys_Printf("%sSpectator %s <%d> removed\n",color[COLOR_CONN], drop->name, drop->userid);
		Sys_Printf("Spectator %s <%d> removed %s\n", drop->name, drop->userid, NET_AdrToString(drop->netchan.remote_address));
	else
	    //Sys_Printf("%sClient %s <%d> removed\n", color[COLOR_CONN], drop->name, drop->userid);
		Sys_Printf("Client %s <%d> removed %s\n", drop->name, drop->userid, NET_AdrToString(drop->netchan.remote_address));
...
Called every frame, prints information to clients about inminent restart and actually restarts server
==================
*/

#define CHECKRESTART_SOUND "buttons/switch02.wav"

void SV_CheckRestart (void)
{
	static double hours_passed, restart_begun_at;
	static int lastsecond;

	if (!sv_restarttime.value) return;

	if (sv_updating.value) return;

	hours_passed = (realtime/60)/60;

	if (hours_passed >= sv_restarttime.value)
	{
		if (!restart_state) // First run
		{
			restart_begun_at = realtime;
			SV_BroadcastPrintf(PRINT_HIGH,"Restart Scheluded server reset every %d hours. Server will restart in 30 seconds...\n",(int)sv_restarttime.value);	
			SV_SoundBroadcast(CHECKRESTART_SOUND,(float)1);
			restart_state = 1;
		}
		else if (restart_state == 1) // 30 to 15 seconds left stage
		{
			if (realtime - restart_begun_at > 14)
			{
				restart_state = 2;
				SV_BroadcastPrintf(PRINT_HIGH,"Restart 15 seconds left for server shutdown\n");	
				SV_SoundBroadcast(CHECKRESTART_SOUND,(float)1);
			}
		}		
		else if (restart_state == 2) // final countdown (10-0 seconds left)
		{
			if (realtime - restart_begun_at >= 20)
			{
				if (lastsecond != (int)(realtime - restart_begun_at))
				{
					SV_BroadcastPrintf(PRINT_HIGH,"Restart %d seconds left\n",(int)(31 - (realtime - restart_begun_at)));	
					lastsecond = (int)(realtime - restart_begun_at);
					SV_SoundBroadcast(CHECKRESTART_SOUND,(float)1);
				}

				if (realtime - restart_begun_at >= 30)
					restart_state = 3;
			}
		}
		else if (restart_state == 3) // Last step, server shutdown
		{
			SV_Restart_f();
			//Cbuf_AddText("pinmsg save\nrestart\n");
			restart_state = 4;
		}			
	}	
}
...
/* OfN
==================
ConvertConsoleStringChars

Converts any special chars on a string by modifying char-by-char its content
==================
*/
...
void ConvertConsoleStringChars(char* st)
{
	...
	while (*st)
	{		
		if ((unsigned char)*st > 31 && (unsigned char)*st < 128) // white char
			; // Nothing
		else if ((unsigned char)*st == 10) // \n
			;
		else if ((unsigned char)*st > 159) // red char
			*st -= 128;
		else if ((unsigned char)*st > 145 && (unsigned char)*st < 156) // special variant number
			*st -= 98;
		else if ((unsigned char)*st > 17 && (unsigned char)*st < 28) // special number
			*st += 30;
		else if ((unsigned char)*st == 16 || (unsigned char)*st == 144)
			*st = '[';
		else if ((unsigned char)*st == 17 || (unsigned char)*st == 145)
			*st = ']';
		else
		{
			switch ((unsigned char)*st)
			{
				case 5:
				case 14:
				case 15:
...
void SetConsoleToUserColor(client_t* theclient)
{
	...
	topcolor = atoi(Info_ValueForKey (theclient->userinfo, "topcolor"));

	switch (topcolor)
	{
	case 0:
		Sys_ConsoleColor(COLOR_WHITE,COLOR_DEFAULT);
		break;
	case 1:
		Sys_ConsoleColor(COLOR_BROWN,COLOR_DEFAULT);
		break;
	case 2:
		Sys_ConsoleColor(COLOR_LBLUE,COLOR_DEFAULT);
		break;
	case 3:
		Sys_ConsoleColor(COLOR_LGREEN,COLOR_DEFAULT);
		break;
	case 4:
		Sys_ConsoleColor(COLOR_LRED,COLOR_DEFAULT);
		break;
	case 5:
		Sys_ConsoleColor(COLOR_YELLOW,COLOR_DEFAULT);
		break;
	case 8:
		Sys_ConsoleColor(COLOR_PINK,COLOR_DEFAULT);
		break;
	case 9:
		Sys_ConsoleColor(COLOR_PINK,COLOR_DEFAULT);
		break;
	...
	default:
		Sys_ConsoleColor(COLOR_DEFAULT,COLOR_DEFAULT);
	}
}
...
Gets information from client infokey, if any, and updates client & version fields
==================
*/
...
{		
	...
	else
	{
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*ProzacQW"));
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*FuhQuake"));
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*QWAMF"));
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*z_ver"));
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*qf_version"));
		...
		if (Info_ValueForKey(theclient->userinfo,"*client")[0])
		{
		...
		versionk = atof(Info_ValueForKey(theclient->userinfo,"*ver"));
...
void SVC_DirectConnect (void)
{
	...
	*newcl = temp;
	...
	Sys_BeginColor();
	Sys_ConsoleColor(COLOR_FORE_CONNECTION,COLOR_BACK_CONNECTION);

	if (newcl->spectator) // kk added ip addr and uid to these 
		//Con_Printf ("%sSpectator %s <%d> connected %s\n", color[COLOR_CONN], newcl->name, newcl->userid, NET_AdrToString (net_from));
		Con_Printf ("Spectator %s <%d> connected %s\n", newcl->name, newcl->userid, NET_AdrToString (net_from));
	else
		//Con_Printf ("%sClient %s <%d> connected %s\n", color[COLOR_CONN], newcl->name, newcl->userid, NET_AdrToString (net_from));
		Con_Printf ("Client %s <%d> connected %s\n", newcl->name, newcl->userid, NET_AdrToString (net_from));
	...
	// KK clc code
	newcl->clcheck = clc_unchecked;
	newcl->clctime = realtime + 30.0;
	newcl->clcnum = rand();
	// KK cuff code
...
qboolean rcon_from_user = false; // is the current command an rcon from user
/*
===============
SVC_RemoteCommand
...
{
	qboolean	do_cmd, admin_cmd;
	char	remaining[MAX_MSGLEN], *name;

	do_cmd = admin_cmd = false;
	...
	Sys_BeginColor();
	Sys_ConsoleColor(COLOR_FORE_ADMIN,COLOR_BACK_ADMIN);

	if (( name = Name_of_sender() ) != NULL ) { 	// log issuer
		if (do_cmd && admin_cmd) {		// admin, broadcast
			dont_sysprint_in_bdcast = true;
			SV_BroadcastPrintf (PRINT_HIGH,
			  "Admin %s issued %s command:\n", name, Cmd_Argv(2));
			Con_Printf(
			"%sAdmin %s issued %s command:\n",
			color[COLOR_RCON], name, Cmd_Argv(2));
		} else if (admin_cmd) 			// bad admin cmd
			Con_Printf("%sAdmin %s issued illegal command:\n",
			color[COLOR_RCON],  name);
		else 					// rcon cmd
			Con_Printf("%sUser %s %s rcon command:\n",
			color[COLOR_RCON], name, do_cmd? "issued":"attempted");
	}

	Sys_ConsoleColor(COLOR_FORE_ADMIN|COLOR_INTENSE,COLOR_BACK_ADMIN);

	Con_Printf ("%s from %s:\n%s\n", do_cmd? "Rcon" : "Bad rcon",
	...
	if (do_cmd) {
		strncpy(remaining,Cmd_Args2(),sizeof(remaining)-1);
...
void SV_ConnectionlessPacket (void)
{
	...
	// KK hostile proggie code
	if ( net_message.cursize > MAX_MSGLEN ) {	// trunc large packets
		Sys_Printf("Oversize ConnectionlessPacket: "
		"size = %d, truncated to %d\n",net_message.cursize,MAX_MSGLEN);
		net_message.cursize = MAX_MSGLEN; 
	}
	net_message.data[net_message.cursize] = 0;	// nullterm *all* pckts
	// KK hostile proggie code
...
void SV_AddIP_f (void)
{
	...
	filttypes[i] = ft_ban;
	ipfilters[i].time = mins? realtime + mins*60.0  :  0.0;
	if (!StringToFilter (Cmd_Argv(1), &ipfilters[i]))
		ipfilters[i].compare = 0xffffffff;
	else
		Con_Printf ("Added IP Filter %s %s\n",Cmd_Argv(1),
			mins? va("for %.1f minutes",mins) : "permanently");
	CleanIPlist();
}
...
void SV_RemoveIP_f (void)
{
	...
	if (Cmd_Argc() < 2) {
		Con_Printf ("usage: removeip <ip>\n");
		return;
	}
	strncpy(tmpstr, va("%s%s%s%s", Cmd_Argv(1),Cmd_Argv(2),Cmd_Argv(3),Cmd_Argv(4)), sizeof(tmpstr)-1);	// allow spaces in ip (same as listip output)
	tmpstr[ sizeof(tmpstr)-1 ] = 0;

	if (!StringToFilter (tmpstr, &f))
		return;
	for (i=0 ; i<numipfilters ; i++) {
...
void SV_ListIP_f (void)
{
	...
	for (i=0 ; i<numipfilters ; i++)
	{
		*(unsigned *)b = ipfilters[i].compare;
		t = "ban ";
		if ( filttypes[i] == ft_cuff ) t = "cuff";
		if ( filttypes[i] == ft_mute ) t = "mute";
		Con_Printf ("%3i.%3i.%3i.%3i   %s %s\n",
		    b[0], b[1], b[2], b[3], t, ipfilters[i].time?
		    va("minutes left=%5.1f", (ipfilters[i].time-realtime)/60.0)
		    : "permanent");
	}
}
...
void SV_WriteIP_f (void)
{
	...
	for (i=0 ; i<numipfilters ; i++)
	{
		if ( filttypes[i] != ft_ban )
			continue;
		*(unsigned *)b = ipfilters[i].compare;
		fprintf (f, "addip %i.%i.%i.%i  %.0f\n", b[0], b[1], b[2], b[3],		ipfilters[i].time? (ipfilters[i].time - realtime)/60.0 : 0.0);
	}
...
sets until to time banned till, 0 for perm
=================
*/
qboolean SV_FilterPacket (double *until)
{
	int		i;
	unsigned	in;
...
void SV_SavePenaltyFilter (client_t *cl, filtertype_t type, double pentime)
{
	...
	ip = *(unsigned *)(cl->netchan.remote_address.ip);
	...
	if ( pentime < realtime ) 	// no point
		return;
	Sys_Printf ("Penalty saved for user=%d\n",cl->userid);
...
void SV_InitLocal (void)
{
	...
	Cvar_RegisterVariable (&sv_maxrate);		// KK hack
	Cvar_RegisterVariable (&sv_cheatpc);		// KK hack
	Cvar_RegisterVariable (&admin_password);	// KK hack

	Cvar_RegisterVariable (&sv_pinsfile); // OfN
	Cvar_RegisterVariable (&sv_crashlog); // OfN
	Cvar_RegisterVariable (&sv_restarttime); // OfN
	Cvar_RegisterVariable (&sv_updating); // OfN
	Cvar_RegisterVariable (&sv_warnmodels); // OfN
	Cvar_RegisterVariable (&con_cleanout); // OfN
	Cvar_RegisterVariable (&con_cleanin); // OfN
	Cvar_RegisterVariable (&con_color); // OfN
...
/*#ifdef _WIN32
	...
	//Info_SetValueForStarKey (svs.info, "*version", va("%4.2f-75.KK %s", VERSION, os), MAX_SERVERINFO_STRING);
	//Info_SetValueForStarKey (svs.info, "*version2", "Admin code KK@rod.net", MAX_SERVERINFO_STRING);
	// OfN - Yes, this source was KK version of QWSV 75 :)
	Info_SetValueForStarKey (svs.info, "*version", va("%4.2f-CPQWSV %s", QWSVVERSION, os), MAX_SERVERINFO_STRING);
...
{
	char		*val, *p, *q;
	int		i, maxrate = (int)sv_maxrate.value;
	client_t	*client;
	char		newname[80];
	qboolean	no_good_chars;
	...
	no_good_chars = true;		// check for one reasonable char
	for (p = newname; *p; p++)
		if ( KK_qwchar(*p) != 'X' ) {
			no_good_chars = false;
			break;
...
//		strcpy(newname, va("user-%d", cl->userid));
	...
	if (realtime < cl->lockedtill && cl->state == cs_spawned)
		// speak-by-name-change
		val = va("muted-%d", cl->userid);
...
//		val = va("user-%d", cl->userid);
		...
		for (i=0, client = svs.clients; i<MAX_CLIENTS ; i++, client++) {
			...
			if (!stricmp(client->name, val)) {
				val = va("%-.25s-%d", val, cl->userid);
				break;
			}
		}
	}
	
	if (strcmp(val, cl->name)) {
		if (!sv.paused) {
		...
		if (cl->state >= cs_spawned && !cl->spectator) {
			...
			/*SV_BroadcastPrintf (PRINT_HIGH,
			"%s changed name to %s\n", cl->name, val);
			Sys_Printf ("%s%s <%d> %schanged name to %s%s <%d>\n",
			KK_Team_Color(cl), cl->name, cl->userid,
			color[COLOR_TEXT], KK_Team_Color(cl), val, cl->userid);*/
			SV_BroadcastPrintf (PRINT_HIGH,
			"%s changed name to %s\n", cl->name, val);
			Sys_Printf ("%s <%d> changed name to %s <%d>\n",
			cl->name, cl->userid, val, cl->userid);
		}
	}
	...
	if (strlen(val))
	{
		...
		if (i > 10000)
			i = 10000;
		if ( maxrate && (i > maxrate) ) {		// KK hack
			Sys_Printf ("Server Rate cap %d to %d for uid=%d\n",
				i, maxrate, cl->userid);
			SV_ClientPrintf (cl, PRINT_HIGH,
				"Server capped rate to %d\n",maxrate);
			KK_stuffcmd (cl, va("rate %d\n",maxrate));
			Info_SetValueForKey (cl->userinfo,
				"rate", va("%d",maxrate), MAX_INFO_STRING);
			i = maxrate;
		} 						// KK hack
...
void SV_Init (quakeparms_t *parms)
{
...
#ifdef _WIN32
	// OfN - Server now raises its process priority under windows
	...
	Cbuf_InsertText ("exec cpserver.cfg\n");
	...
	Con_Printf ("\nProzac Server Version %4.2f (QW %4.2f) (Build %04d)\n\n", CPSVVERSION,QWSVVERSION, build_number());

	//Con_Printf ("======== QuakeWorld Initialized ========\n");
	Con_Printf ("==== PROZAC QuakeWorld server ready ====\n");
	
// process command line arguments
	Cmd_StuffCmds_f ();
	Cbuf_Execute ();
	
	// If color console is enabled, show a fancy coloured message ^_^
	if (con_color.value)
	{
		Sys_BeginColor();
		...
		Sys_ConsoleColor(COLOR_LGREEN,COLOR_DEFAULT);
		Sys_Printf("l");
		Sys_ConsoleColor(COLOR_TEAL,COLOR_DEFAULT);
		Sys_Printf("o");
		Sys_ConsoleColor(COLOR_LBLUE,COLOR_DEFAULT);
		Sys_Printf("r");
		...
		Sys_ConsoleColor(COLOR_LRED,COLOR_RED);
		Sys_Printf("e");
		Sys_ConsoleColor(COLOR_YELLOW,COLOR_BROWN);
		Sys_Printf("n");
		Sys_ConsoleColor(COLOR_LGREEN,COLOR_GREEN);
		Sys_Printf("a");
		Sys_ConsoleColor(COLOR_TEAL,COLOR_CYAN);
		Sys_Printf("b");
		Sys_ConsoleColor(COLOR_LBLUE,COLOR_BLUE);
		Sys_Printf("l");
		Sys_ConsoleColor(COLOR_PINK,COLOR_MAGENTA);
		Sys_Printf("e");
		Sys_ConsoleColor(COLOR_LRED,COLOR_RED);
		Sys_Printf("d");
		...
		//Sys_ConsoleColor(COLOR_RESET,COLOR_DEFAULT);
		Sys_Printf("\n");
		
		Sys_EndColor();
	}

	// Report clean output for console if enabled
	if (con_cleanout.value)
#ifdef _WIN32
		Sys_Printf("Command prompt and clean output enabled.\n");
#else
		Sys_Printf("Clean output enabled.\n");
#endif

	Sys_Printf("Type \"help\" for informative commands.\n");
